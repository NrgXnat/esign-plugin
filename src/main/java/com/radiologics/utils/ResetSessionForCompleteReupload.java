 ...
 *
 *  ReuploadComplete operation will change the status to quarantine and then delete everything from the session.
 *
 */
public class ResetSessionForCompleteReupload  implements Callable<XnatExperimentdata>{
	 ...
	 */
	public ResetSessionForCompleteReupload(UserI user, XnatSubjectassessordata exp, EventDetails details){
		this.user=user;
		this.exp=exp;
		this.details=details;
	}
	private boolean checkPermissions(final ArchivableItem i, final UserI user) throws Exception{
	...
	public XnatExperimentdata call() throws Exception {
		...
		if(BooleanUtils.toBooleanDefaultIfNull(siteConfig.getBooleanValue("esignSendCompleteReuploadEmail"), false)){
			this.sendEmail();
		}
		return exp;
	}
	...
	public void sendEmail() throws Exception{
		...
		final String subjectTemplate = (String) siteConfig.getProperty("esignCompleteReuploadEmailSubject", EsignEmailHelper.DEFAULT_COMPLETE_REUPLOAD_SUBJECT);
		final String bodyTemplate    = (String) siteConfig.getProperty("esignCompleteReuploadEmailBody", EsignEmailHelper.DEFAULT_COMPLETE_REUPLOAD_EMAIL);
