 ...
 *
 * Prepare a session for complete reupload.  The post method adds a workflow entry, deletes all of its files and deletes all metadata expect the ID, label, and project 
 * and sets the session status to quarantine.
 */
@XnatRestlet("/services/esig/complete")
public class ReuploadCompleteService extends SecureResource {
	 ...
	 */
	public ReuploadCompleteService(Context context, Request request, Response response) {
	...
	public void handlePost() {
		...
		try {
			(new ResetSessionForCompleteReupload(this.getUser(), (XnatSubjectassessordata) exp, this.newEventInstance(EventUtils.CATEGORY.DATA,"Reset Session And Move To Quarantine"))).call();
